...
async def main(SCALE):
    ...
    while True:
        ...
        display.fill("white")
        for snake in snakes_update:
            sight = snake["sight"]  # Sight is a nested dictionary

            # Iterate through the sight dictionary
            for x_str, y_dict in sight.items():
                x = int(x_str)  # Convert x-coordinate to an integer
                for y_str, tile_value in y_dict.items():
                    y = int(y_str)  # Convert y-coordinate to an integer
